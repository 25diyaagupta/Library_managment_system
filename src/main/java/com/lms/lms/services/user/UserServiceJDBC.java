...
public class UserServiceJDBC implements UserService {
    ...
    public List<Map<Integer, Users>> getAllIssuedUsers() throws SQLException {
        String query = " select * from users u join issue_return ir on ir.user_id =u.id join books b on ir.book_id =b.book_id where status = 'Issued'";
        return jdbcTemplate.query(query, new ResultSetExtractor<List<Map<Integer, Users>>>() {
            @Override
            public List<Map<Integer, Users>> extractData(ResultSet rs) throws SQLException, DataAccessException {
                ...
                List<Map<Integer, Users>> resultList = new ArrayList<>();
                resultList.add(map);
                return resultList;
            }
        });
